...
/** Recursively search elements (and column_set `columns`) for a given tag. */
function hasTag(elements: ElementShape[] | undefined, tag: string): boolean {
  if (!elements) return false;
  for (const e of elements) {
    if (e.tag === tag) return true;
    const children = e.elements ?? (e as { columns?: ElementShape[] }).columns;
    if (hasTag(children, tag)) return true;
  }
  return false;
}
...
describe('renderCard tool display modes', () => {
  ...
  it('hide mode while running still shows the stop button and footer', () => {
    ...
    // The stop button is wrapped in an auto-width column_set (bottom-left
    // layout), so look for the button tag recursively rather than at top level.
    expect(hasTag(card.body?.elements, 'button')).toBe(true);
  });
