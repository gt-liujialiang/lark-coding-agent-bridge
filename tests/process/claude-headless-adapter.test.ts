...
describe('ClaudeHeadlessAdapter process contract', () => {
  ...
  it('spawns a fresh run with stream-json, verbose, permission mode, and bridge prompt args', async () => {
    ...
    const run = new ClaudeHeadlessAdapter({ binary: fake.path }).run({
      runId: 'run-fresh',
      prompt: 'hello',
      cwd: fake.dir,
  ...
  it('injects the active bridge profile env into spawned runs', async () => {
    ...
    const run = new ClaudeHeadlessAdapter({
      binary: fake.path,
      larkChannel: {
        profile: 'codex-dev',
        rootDir,
  ...
  it('surfaces spawn errors as stream error events', async () => {
    let run: ReturnType<ClaudeHeadlessAdapter['run']>;
    if (process.platform === 'win32') {
      ...
      cleanup.push(fake.dir);
      run = new ClaudeHeadlessAdapter({ binary: fake.path }).run({
        runId: 'run-missing',
        prompt: 'hi',
        cwd: fake.dir,
      });
      ...
      run = new ClaudeHeadlessAdapter({ binary: missing }).run({
        runId: 'run-missing',
        prompt: 'hi',
        cwd: tmpdir(),
      });
  ...
  it('requires cwd to be resolved by policy before spawning', () => {
    expect(() =>
      new ClaudeHeadlessAdapter({ binary: 'unused' }).run({ runId: 'run-no-cwd', prompt: 'hi' }),
